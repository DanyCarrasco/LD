...
asegurar_ronda -->
    state(S0, S),
    {
        ...
        ( select(ronda(Resultados), S0, S1) ->
            ...
            S = [ronda(Resultados, ninguno, none, EstadoEnvido, EstadoTruco, none)|S1]
        ; select(ronda(Resultados, C), S0, S1) ->
            estado_envido_inicial(EstadoEnvido),
            S = [ronda(Resultados, C, none, EstadoEnvido, EstadoTruco, none)|S1]
        ; select(ronda(Resultados, C, R), S0, S1) ->
            estado_envido_inicial(EstadoEnvido),
            estado_cantos_Truco(EstadoTruco),
            S = [ronda(Resultados, C, R, EstadoEnvido, EstadoTruco, none)|S1]
        ; S = S0
        )
    }.
...
set_ronda_canto(Canto) -->
    state(S0, S),
    { select(ronda(Resultados, _, R, E, T, P), S0, S1),
      S = [ronda(Resultados, Canto, R, E, T, P)|S1] }.

% marca rechazo en la ronda
set_rechazo(Jug) -->
    state(S0, S),
    { select(ronda(Resultados, C, _, E, T, P), S0, S1),
      S = [ronda(Resultados, C, rechazo(Jug), E, T, P)|S1] }.

% cambia el estado del envido
set_estado_envido(EstadoEnvido) -->
    { format("seteando el estado del envido",[]) },
    state(S0, S),
    { select(ronda(Resultados, C, R, _, T, P), S0, S1),
      S = [ronda(Resultados, C, R, EstadoEnvido, T, P)|S1] }.

% cambia el jugador pendiente de jugar carta
set_pendiente(Pendiente) -->
    state(S0, S),
    { select(ronda(Resultados, C, R, E, T, _), S0, S1),
      S = [ronda(Resultados, C, R, E, T, Pendiente)|S1] }.

% avanza el estado de los cantos de truco
step_estado_truco -->
    state(S0,S),
    { select(ronda(Resultados,C,R,E,trucos(X),P),S0,S1),
    (X = [truco]->
        T1 = [retruco]
    ; (X = [retruco]) ->
        T1 = [vale4]
    ...
    S = [ronda(Resultados,C,R,E,trucos(T1),P)|S1]
    }.
