...
%
% ronda:
%   ronda(resultados, canto_actual, rechazo, estado_envido, estado_truco, pendiente)
%
% estado_envido:
...
jugar_truco -->
    ...
    % ESTA ES LA ÚNICA LÍNEA NUEVA AÑADIDA:
    interfaz_web:notificar_manos_web,
...
turno_jugador(Nombre, CartaJugada, TerminaRonda) -->
    state(S0, S0),
    {
        ...
        entrada_web(Nombre, "Elegi accion", [jugar, cantar], Accion)
    },
    ({ Accion == jugar } ->
        {
          format("~w: ", [Nombre]),
          entrada_web(Nombre, "elegi carta", Mano, Carta),
          CartaJugada = Carta,
          TerminaRonda = no
        }
    ;   % Si la accion no es jugar, se asume que es cantar
        ...
        {
          entrada_web(Nombre, "canta", Opciones, Canto)
        },
...
resolver_mano_cartas(P0, CartasSeleccionadas) -->
    state(S0, S),
    {
        ...
        S = [ronda(Resultados1, CantoActual, Rech, EstadoEnvido, EstadoTruco, Pendiente), jugadores(P1)|S2]
    }.
...
puede_cantar_envido_estado(Nuevo) -->
    state(S0, S0),
    {
        select(ronda([], _, none, envido(no_cantado, Cantos, none), _, _), S0, _),
