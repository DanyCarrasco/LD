...
%
% ronda:
%   ronda(resultados, canto_actual, rechazo, estado_envido, estado_truco)
%
% estado_envido:
...
start -->
    state(_, [mazo(Cartas), ronda([], ninguno, none, EstadoEnvido, EstadoTruco)]),
    {
...
jugar_truco -->
    state(S0, S),
    {
        ...
        select(ronda(_, _, _, _, _), S1, S2),
        maplist(nueva_mesa, P0, P1),
        cambiar_mano(P1, P2),
        imprimir_puntajes_inicio_mesa(P2),
        estado_envido_inicial(EstadoEnvido),
        estado_cantos_Truco(EstadoTruco),
        S = [ronda([], ninguno, none, EstadoEnvido, EstadoTruco), jugadores(P2)|S2]
    },
...
jugar_mesa -->
    jugar_mano,
    state(S, S),
    { select(ronda(Resultados, _, Rech, _, _), S, _) },
...
turno_jugador(Nombre, CartaJugada, TerminaRonda) -->
    ...
    ;   % Si la accion no es jugar, se asume que es cantar
...
resolver_canto_o_envido_en_turno(Nombre, Canto, TerminaRonda) -->
    ...
    ;
        ...
        state(S1, S1),
        { ( select(ronda(_, _, rechazo(_), _, _), S1, _) ->
            TerminaRonda = si
        ;
            TerminaRonda = no
        ) }
...
resolver_envido_en_turno(J, Canto) -->
    ( puede_cantar_envido_estado(Canto) ->
        state(S, S),
        {
            select(ronda(_, _, _, envido(_, CantosPrevios, _),_), S, _),
...
resolver_canto_en_turno(J, Canto) -->
    ...
    ;
        { format("~w: Canto invalido o no permitido.~n", Canto) }
    ).
...
resolver_respuesta_canto(Cantor, Rival, Canto, Resp) -->
    ...
    ; { es_canto_envido(Resp) } ->
        state(S, S),
        {
            select(ronda(_, _, _, envido(_, Cantos, _), _), S, _),
            canto_envido_valido(Cantos, Resp),
            format("~w canta ~w~n", [Rival, Resp]),
            append(Cantos, [Resp], CantosNuevos)
        },
        pedir_respuesta_envido(Cantor, Resp2),
        resolver_respuesta_envido(Rival, Cantor, CantosNuevos, Resp2)
...
resolver_mano_cartas(P0, CartasSeleccionadas) -->
    state(S0, S),
    {
        format("resolviendo cartas!!",[]),
        select(jugadores(P0), S0, S1),
        select(ronda(Resultados, CantoActual, Rech, EstadoEnvido, EstadoTruco), S1, S2),
        ...
        S = [ronda(Resultados1, CantoActual, Rech, EstadoEnvido, EstadoTruco), jugadores(P1)|S2]
    }.
...
finalizar_ronda -->
    state(S0, S),
    {
        select(ronda(Resultados, Canto, Rech, envido(_Estado, _Cantos, _PremioEnvido),_), S0, S1),
        ...
        estado_cantos_Truco(EstadoTruco),
        S = [ronda([], ninguno, none, EstadoEnvidoNuevo, EstadoTruco), jugadores(P1)|S2]
    }.
...
puede_cantar_estado(Nuevo) -->
    state(S0, S0),
    {
        select(ronda(_, CantoActual, none, _, _), S0, _),
        es_canto(Nuevo),
...
puede_cantar_envido_estado(Nuevo) -->
    state(S0, S0),
    {
        select(ronda([], _, none, envido(no_cantado, Cantos, none), _), S0, _),
