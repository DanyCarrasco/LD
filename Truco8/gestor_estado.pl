...
asegurar_ronda -->
    state(S0, S),
    {
        ...
        ;select(ronda(Resultados, C, R), S0, S1) ->
            ...
            S = [ronda(Resultados, C, R, EstadoEnvido, EstadoTruco)|S1]
        ; S = S0
        )
    }.
...
set_estado_envido(EstadoEnvido) -->
    ...
    state(S0, S),
    { select(ronda(Resultados, C, R, _, T), S0, S1),
      S = [ronda(Resultados, C, R, EstadoEnvido,T)|S1] }.


step_estado_truco -->
    state(S0,S),
    { select(ronda(Resultados,C,R,E,trucos(X)),S0,S1),
    (X = [truco]->
        T1 = [retruco]
    ; (X = [retruco]) ->
        T1 = [vale4]
    ...
    % format("llamado a step estado, paso de ~w a ~w~n", [X,T1]),
    S = [ronda(Resultados,C,R,E,trucos(T1))|S1]
    }.
